...
module \$_SDFFCE_PP0P_ (input D, C, R, E, output Q);
    ...
    sky130_fd_sc_hd__and2_1 AND_gate_rst
    (.A(reset_output), .B(C), .X(gated_rst));
    
    sky130_fd_sc_hd__and2_1 AND_gate_clk.gated_en
    (.A(enable_output), .B(C), .X(gated_clk));
    
    \$_MUX_ mux_rst_inst 
    (.Y(mux_output), .A(connector), .B(1'b0), .S(gated_rst));
...
module \$_SDFFCE_PP1P_ (input D, C, R, E, output Q);
    ...
    \$_MUX_ mux_rst_inst 
    (.Y(mux_output), .A(connector), .B(1'b1), .S(gated_rst));
...
module \$_SDFFCE_PN0P_ (input D, C, R, E, output Q);
    ...
    \$_MUX_ mux_rst_inst 
    (.Y(mux_output), .A(1'b0), .B(connector), .S(gated_rst));
...
module \$_DFFE_PN1P_ (input D, C, R, E, output Q);
    ...
    sky130_fd_sc_hd__or2_1 OR_gate_en_rst
    (.A(enable_output), .B(inv_reset_output), .X(or_en_rst));

    sky130_fd_sc_hd__and2_1 AND_gate_clk.gated_en
    (.A(or_en_rst), .B(C), .X(gated_clk_2_en));

    sky130_fd_sc_hd__or2_1 OR_gate_clk
    (.A(gated_clk_2_en), .B(inv_reset_output), .X(gated_rst));
    
    \$_MUX_ mux_rst_inst 
    (.Y(mux_output), .A(1'b1), .B(connector), .S(reset_output));
    
    \$_DFF_P_ _TECHMAP_REPLACE_.custom_gated_FF_2 
    (.D(mux_output), .C(gated_rst), .Q(Q));
endmodule
